...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        centre = self.find_line(cv_image)
        if centre < 0:
            self.timeout += 1
        else:
            self.timeout = 0
        ...
        bin_width = cv_image.shape[1] / len(state)
        state[int(centre / bin_width)] = 1
        return state, done
    
    def find_line(self, cv_image):
        '''
            @brief Finds the line in the image and returns the x coordinate of its centre
            @param cv_image : opencv image

            @retval x : x coordinate of the line, returns -1 if no line is found
        '''
        # Convert the frame to grayscale
        gray = cv2.cvtColor(cv_image, cv2.COLOR_BGR2GRAY)

        # Apply a binary threshold (110.0) that I found with Otsu's
        (thresh, bw) = cv2.threshold(gray, 110.0, 255, cv2.THRESH_BINARY)

        # Find the positions of white pixels in a specific row (line detection)
        high_positions, = np.where(bw[235, :] == 0)
